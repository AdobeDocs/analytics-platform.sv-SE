...
Customer Journey Analytics bearbetar rapporter när de körs och distribuerar den kombinerade datauppsättningen till flera servrar. Storleken på databegäran och mängden tillgänglig Adobe-maskinvara är två av många faktorer som hjälper till att avgöra hur många servrar som har tilldelats för att bearbeta en rapport. Eftersom servrar tilldelas dynamiskt och inte visas i gränssnittet går det inte att se eller styra antalet servrar som bearbetar en rapport.
...
Även om vissa enskilda dimensionsobjekt saknas i kardinalrapporterna är kolumnsummorna korrekta och inte baserade på trunkerade data. Den [[!UICONTROL Approximate Count Distinct]](/help/components/calc-metrics/cm-adv-functions.md#approximate-count-distinct) beräknade måttfunktionen påverkas inte heller av trunkerade dimensionsobjekt.
...
* Om antalet unika värden är primärt fokus använder du den [[!UICONTROL Approximate Count Distinct]](/help/components/calc-metrics/cm-adv-functions.md#approximate-count-distinct) beräknade måttfunktionen.
